...
internal static class SessionEndpoints
{
    ...
    private static async Task<Results<Created<Session>, BadRequest<ErrorResponse>, InternalServerError<ErrorResponse>>>
        CreateSession(
            Session session,
            ...
            ILogger<SessionEndpointsLogger> logger)
    {
        ...
        {
            return TypedResults.BadRequest(Errors.ProtocolVersionIsNotSupported.ToResponse());
        }
        ...
        switch (error)
        {
            case Errors.IClientError:
                return TypedResults.BadRequest(error.ToResponse());
            case Errors.IServerError:
                return TypedResults.InternalServerError(error.ToResponse());
            default:
            {
                var unexpectedError =
                    new ErrorResponse(new ErrorDetail("UnexpectedError", "Unable to create a session"));
                return TypedResults.InternalServerError(unexpectedError);
            }
        }
    }
    ...
    private static async Task<Results<Ok, NoContent, BadRequest<ErrorResponse>, InternalServerError<ErrorResponse>>>
        DeleteSession(
            string sessionId,
            [FromQuery(Name = "api-version")] string apiVersion,
            [FromHeader(Name = "Microsoft-Developer-DCP-Instance-ID")]
            ...
            ILogger<SessionEndpointsLogger> logger)
    {
        ...
        switch (error)
        {
            case Errors.AspireSessionNotFoundError:
            ...
            {
                var unexpectedError =
                    new ErrorResponse(new ErrorDetail("UnexpectedError", "Unable to delete a session"));
                return TypedResults.InternalServerError(unexpectedError);
            }
        }
    }
