...
class BuildAspireFileBeforeRunTaskProvider: BeforeRunTaskWithProjectProvider<BuildAspireFileBeforeRunTask>()  {
  companion object {
    val providerId = Key.create<BuildAspireFileBeforeRunTask>("Build")

    @Nls
    private val TASK_NAME = RiderBuildBundle.message("BuildProjectBeforeRunTaskProvider.task.name.build.project")
    val logger = getLogger<BuildAspireFileBeforeRunTaskProvider>()
  }
  ...
  override fun executeTask(
    ...
    if (configuration !is AspireFileConfiguration) {
      logger.warn("configuration is not AspireFileConfiguration")
      return false
    }
