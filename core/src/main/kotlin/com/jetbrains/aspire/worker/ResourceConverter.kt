...
import com.jetbrains.aspire.generated.ResourceEnvironmentVariable
...
import com.jetbrains.aspire.generated.ResourceProperty
import com.jetbrains.aspire.generated.ResourceRelationship
...
import com.jetbrains.aspire.generated.ResourceUrl
import com.jetbrains.aspire.generated.ResourceVolume
...
internal fun ConsoleLogLine.toResourceLog(): ResourceLog? {
    if (text.isEmpty()) return null
    return ResourceLog(text, if (hasIsStdErr()) isStdErr else false, lineNumber)
}
...
private fun mapResourceStateStyle(style: String): ResourceStateStyle = when (style.lowercase()) {
...
private fun getStringValue(value: Value): String? = when (value.kindCase) {
    Value.KindCase.STRING_VALUE -> value.stringValue
    Value.KindCase.BOOL_VALUE -> value.boolValue.toString()
    Value.KindCase.NUMBER_VALUE -> value.numberValue.toBigDecimal().toPlainString()
    Value.KindCase.NULL_VALUE, Value.KindCase.KIND_NOT_SET -> null
    else -> value.toString()
}

private fun com.jetbrains.aspire.generated.dashboard.EnvironmentVariable.toResourceEnvironmentVariable():
